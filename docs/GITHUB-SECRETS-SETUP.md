...
### 0.1 Tổng quan — 9 secrets + 2 variables
...
| `CLOUDFLARE_API_TOKEN` | Secret | CF token (DNS Edit + Pages Edit) | Caddy TLS cert + Frontend deploy |
...
- Deploy workflow tự động inject `POSTGRES_PASSWORD`, `JWT_SECRET`, `CLOUDFLARE_API_TOKEN` vào `.env` trên server — không cần SSH thủ công
